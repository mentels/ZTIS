...
crawlAllProvinces = do
    ...
    save provinces "regiosetProvincesCrawled.txt" where
...
readProvinceFromPage tags = do
    ...
    let countiesList = filter (not . DLU.startswith "Miasto" . countyName) $ map readCountyFromPage countiesPages
...
readCountyFromPage :: [Tag String] -> County
readCountyFromPage tags = County {countyName = cn, neighbors = ns, councillors = []} where
